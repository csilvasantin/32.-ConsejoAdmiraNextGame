/* app.js — extraído de index.html para lazy-load (defer). Lógica idéntica, mismo orden y scope. */
    ...
    async function toggleHack() {
        ...
        if (!hackMode) {
            // === ACTIVATE HACK ===
            // Token de hackeo vive solo en demo-server (Mini). El navegador no lo pide ni lo guarda.
            hackMode = true;
            btn.classList.add("active");
            ...
            try {
                ...
                console.warn("HACK: demo-server /hackeo error", e.message);
            }
    ...
    // Proxy demo-server: POST HACK_API /hackeo. Sin token en el navegador.
    async function callCouncilHackeo(mode) {
        const path = mode === "stop" ? "/hackeo/stop" : "/hackeo";
        const body = JSON.stringify({ only_ids: [], exclude_ip: MY_TAILNET_IP || "" });
    ...
    function showCliHelp() {
        ...
        document.getElementById('win-racional').innerHTML =
            ...
            '<li><strong>/mac on|off|toggle</strong> — muestra u oculta el Macintosh 1984 (misiones Hoy)</li>' +
    ...
    function handleCliCommand(raw) {
        ...
        if (macMatch) {
            ...
            setActionLine(api.isVisible() ? '🖥 Macintosh Hoy — CRT con misiones vivas · /mac off limpia la mesa' : '🖥 mesa limpia — /mac o MOSTRAR para encender el CRT');
            return true;
        }
